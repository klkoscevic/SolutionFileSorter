...
namespace UnitTests
{
    ...
    public class ProjectEntryComparerTests
    {
        ...
        public void CompareReturnsNameComparisonResultForTwoSolutionFolderEntriesInTheRoot()
        {
            ...
            var pe2 = new ProjectEntry("abd", "guid", true, Range.Empty);
            Assert.IsTrue(comparer.Compare(pe1, pe2) < 0);

            var pe3 = new ProjectEntry("abcd", "guid", true, Range.Empty);
            Assert.IsTrue(comparer.Compare(pe1, pe3) < 0);

            var pe4 = new ProjectEntry("abc", "guid", true, Range.Empty);
            Assert.IsTrue(comparer.Compare(pe1, pe4) == 0);

            var pe5 = new ProjectEntry("abc c", "guid", true, Range.Empty);
            Assert.IsTrue(comparer.Compare(pe1, pe5) < 0);

            var pe6 = new ProjectEntry("abc d", "guid", true, Range.Empty);
            Assert.IsTrue(comparer.Compare(pe5, pe6) < 0);
        }

        [TestMethod]
        public void ComparePlacesSolutionFolderInFrontOfAnyProjectEntry()
        {
            var pe1 = new ProjectEntry("abc", "guid", false, Range.Empty);
            var pe2 = new ProjectEntry("def", "guid", true, Range.Empty);
            Assert.IsTrue(comparer.Compare(pe1, pe2) > 0);
        }
        ...
        [TestMethod]
        public void CompareReturnsResultOfParentComparisonIfTheyAreDifferent()
        {
            var pe1 = new ProjectEntry("abc", "guid", true, Range.Empty);
